...
/** The recommended action (§4) — a verb the recipient can act on, from the tone
 *  alone so it never becomes SOCDesk's own verdict. */
export function recommendedAction(tone) {
  return {
    red: "Escalate — verify the strongest signals below, then treat as adverse pending verification.",
    amber: "Verify — reputation is mixed; confirm the flagged sources before any action.",
    green: "Monitor — no adverse findings, but this is not a clearance; corroborate before closing.",
    grey: "Verify — no reputation data returned; check the sources directly before acting.",
  }[tone] ?? "Verify — corroborate with the sources below before acting.";
}
...
function tallyLines(result, now = new Date()) {
  ...
  lines.push(
    "",
    `RECOMMENDED ACTION: ${recommendedAction(result.tone)}`,
    "",
    "CAVEAT: Reputation data can be stale or wrong and has not been independently " +
      "verified; verify before any blocking or client-facing action.",
    `— Generated by SOCDesk ${stampUTC(now)}. Sources queried ${stampUTC(result.checked_at ?? now)}.`,
  );
  return lines;
}
