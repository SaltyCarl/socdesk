...
/* ---- phase 3: collect (await everything — behavior-preserving for now) ---
 * Track B1 Task 2: reproduces today's behavior byte-for-byte. Every source,
 * blocking AND non-blocking, is awaited to completion; a non-blocking
 * failure is routed into `errors` exactly like a blocking one; `partial` is
 * `errors.length > 0`. The grace race (drop a still-PENDING context source
 * at verdict-settle) and the `skipped_context` split arrive in Tasks 3–4. */
...
/* ---- phase 4: assemble (shape unchanged; skipped_context arrives Task 4) - */
