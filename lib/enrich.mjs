...
export async function enrich(fetchImpl, type, q, env = {}, now = new Date()) {
  ...
  // Track B1 note: this task ONLY introduces the `blocking` axis and the
  // `planSources` phase — it still assembles the OLD way over `plan.usable`,
  // so behavior is byte-for-byte unchanged. The dispatch/collect/grace-race
  // split that actually lets OTX/RDAP ride along non-blocking arrives in
  // later tasks.
  const settled = await Promise.allSettled(
    plan.usable.map(s => s.run(fetchImpl, ind, env[s.key])));

  const sources = [], errors = plan.skipped.map(({ source, reason }) => ({ source, reason }));
  settled.forEach((r, i) => {
    if (r.status === "fulfilled") sources.push(r.value);
    else errors.push({
      source: plan.usable[i].name,
...
export const _internals = { SOURCES, b64url, planSources };
