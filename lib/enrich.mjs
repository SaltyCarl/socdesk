...
// RDAP bootstraps through rdap.org and then follows a redirect to the
// authoritative registry server — two sequential round-trips under one budget.
// From the Workers edge that occasionally exceeds the verdict-source budget and
// times out, blanking the registration-age tell. It is CONTEXT (never a
// verdict), so a longer budget costs nothing but worst-case latency on the
// domain lookup — worth it to keep the newly-registered signal reliable.
const RDAP_TIMEOUT_MS = 7000;
// OTX throttles datacenter/Cloudflare egress (fast from a residential IP, slow
// from the Worker). It is context, so a longer budget only costs worst-case
// latency — sized to actually reach OTX rather than false-timeout at 4.5s.
const OTX_TIMEOUT_MS = 9000;
...
const SOURCES = [ABUSEIPDB, VIRUSTOTAL, GREYNOISE, MALWAREBAZAAR, IPINFO, URLSCAN, RDAP, OTX];
