...
const VIRUSTOTAL = {
  ...
  async run(fetchImpl, ind, key) {
    ...
    // Domain-only: VT's domain report carries a `last_dns_records` list
    // ({type, value, ttl}). Surface the first A record so the escalation card
    // can offer a one-click pivot to the hosting IP's own reputation. Never
    // fabricated — missing/empty/no-A-record all fall through to `undefined`
    // and the fact is simply omitted.
    const aRecord = ind.type === "domain"
      ? (Array.isArray(a.last_dns_records) ? a.last_dns_records : [])
          .find(r => r && r.type === "A" && typeof r.value === "string" && r.value.trim())
      : undefined;
    ...
    return {
      ...
      facts: [
        ...
        ...(aRecord ? [["Resolves to", aRecord.value]] : []),
      ],
      url: pivot,
    };
  },
