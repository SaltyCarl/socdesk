 ...
 * Track B1 Task 3: await ONLY the blocking set, then start the grace timer
 ...
 * speed, never waiting on the OTX/RDAP floor). In THIS task both PENDING
 * (slow) and an in-grace `ok:false` (fast fail) are simply omitted; Task 4
 * splits them into `skipped_context` vs `errors` and recomputes `partial`
 * from the blocking axis. `partial` stays `errors.length > 0` for now. */
