...
describe('blocking axis + planSources (Track B1)', () => {
  ...
  it('partitions ipv4 sources: ipinfo blocking, OTX non-blocking', () => {
    ...
    expect(plan.blocking.map((s) => s.name)).toEqual(['AbuseIPDB', 'VirusTotal', 'GreyNoise', 'ipinfo', 'SOCDesk Community'])
  ...
  it('partitions domain sources: RDAP + OTX non-blocking, in SOURCES order', () => {
    ...
    expect(plan.usable.map((s) => s.name)).toEqual(['VirusTotal', 'urlscan', 'RDAP', 'AlienVault OTX', 'SOCDesk Community'])
  })
...
/** Only the VT domain-report URL answers; everything else 404s cleanly (so the
 *  other domain sources return "unknown" instead of throwing). */
function mockVtDomain(attributes) {
  return async (url) => {
    if (String(url).includes('virustotal.com/api/v3/domains/'))
      return { status: 200, ok: true, json: async () => ({ data: { attributes } }) }
    return { status: 404, ok: false, json: async () => ({}) }
  }
}

describe('VirusTotal domain A-record → hosting-IP pivot fact', () => {
  it('exposes the first A record as a "Resolves to" fact', async () => {
    const out = await enrich(mockVtDomain({
      last_analysis_stats: { malicious: 0, suspicious: 0, harmless: 10, undetected: 5 },
      last_dns_records: [
        { type: 'MX', value: 'mail.evil.test', ttl: 300 },
        { type: 'A', value: '203.0.113.7', ttl: 300 },
        { type: 'A', value: '203.0.113.8', ttl: 300 }, // second A record — first wins
      ],
    }), 'domain', 'evil.test', { VT_API_KEY: 'k' })
    const vt = out.sources.find((s) => s.name === 'VirusTotal')
    const facts = Object.fromEntries(vt.facts)
    expect(facts['Resolves to']).toBe('203.0.113.7')
  })

  it('omits the fact when last_dns_records has no A record', async () => {
    const out = await enrich(mockVtDomain({
      last_analysis_stats: { malicious: 0, suspicious: 0, harmless: 10, undetected: 5 },
      last_dns_records: [{ type: 'AAAA', value: '2001:db8::1', ttl: 300 }, { type: 'MX', value: 'mail.evil.test', ttl: 300 }],
    }), 'domain', 'evil.test', { VT_API_KEY: 'k' })
    const vt = out.sources.find((s) => s.name === 'VirusTotal')
    const facts = Object.fromEntries(vt.facts)
    expect(facts['Resolves to']).toBeUndefined()
  })

  it('omits the fact when last_dns_records is absent entirely', async () => {
    const out = await enrich(mockVtDomain({
      last_analysis_stats: { malicious: 0, suspicious: 0, harmless: 10, undetected: 5 },
    }), 'domain', 'evil.test', { VT_API_KEY: 'k' })
    const vt = out.sources.find((s) => s.name === 'VirusTotal')
  ...
  it('omits the fact when last_dns_records is an empty array', async () => {
    const out = await enrich(mockVtDomain({
      last_analysis_stats: { malicious: 0, suspicious: 0, harmless: 10, undetected: 5 },
      last_dns_records: [],
    }), 'domain', 'evil.test', { VT_API_KEY: 'k' })
  ...
  it('never surfaces the fact for a non-domain type (e.g. an IPv4 lookup)', async () => {
    // Same VT payload shape reused for an ipv4 lookup — last_dns_records must
    // never leak an unrelated "Resolves to" fact onto an IP card.
    const out = await enrich(async (url) => {
      if (String(url).includes('virustotal.com/api/v3/ip_addresses/'))
        return {
          status: 200, ok: true, json: async () => ({
            data: {
              attributes: {
                last_analysis_stats: { malicious: 0, suspicious: 0, harmless: 10, undetected: 5 },
                last_dns_records: [{ type: 'A', value: '203.0.113.7', ttl: 300 }],
              },
            },
          }),
        }
      return { status: 404, ok: false, json: async () => ({}) }
    }, 'ipv4', '1.2.3.4', { VT_API_KEY: 'k' })
    const vt = out.sources.find((s) => s.name === 'VirusTotal')
    const facts = Object.fromEntries(vt.facts)
    expect(facts['Resolves to']).toBeUndefined()
  })
