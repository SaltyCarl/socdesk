...
- **⚠ OWNER DECISION — vendor-blog Tier-3 depth.** For active crews with NO TLP:CLEAR HC3/CISA/FBI profile (checked-and-omitted 2026-08-25: DragonForce, INC Ransom, Coinbase Cartel, Kazu, The Gentlemen, MetaEncryptor), the only remaining depth source is **atomic facts (CVE / ATT&CK-technique / alias / tool ids, dates) extracted from reputable vendor blogs WITH per-fact attribution** — defensible under *Feist* (facts aren't copyrightable, same doctrine already used for leak-site claims), but ONLY atomic fields, NEVER paraphrased narrative or a wholesale list. Pushes past public-domain → needs owner's explicit YES. Ready to build on approval (additive to `data/ransomware_intel.json`; the render already handles any seed entry + is source-aware).
