...
- `Lookup.tsx` also **reads** the `#q=` hash directly in two more unguarded spots — the initial `useState(readLookupQuery)` and the `hashchange`/`popstate` sync handler — so a bookmarked/shared `/lookup#q=<command-shaped text>` link fires straight into `useLookup` with no interaction to intercept. These are guarded surfaces too, on top of the `runLookup` submit path, and must not be dropped in a future pass.
