/* ============================================================
   SOCDesk — "System Mug" brand PREVIEW · hero behaviour.
   Ported verbatim from hero-globe.js; the ONLY changes are the
   theme machinery (removed — this sheet is dark-only) and the globe
   palette (re-pointed to ELECTRIC dots on a VOID ground, halo killed).
   cobe globe + drag inertia, reduced-motion static frame, vanilla rAF
   count-up, IntersectionObserver reveal/melt fallback, fly-to spring +
   glass escalation card. No JS animation library.

   CSP-safe: every runtime style write goes through element.style
   .setProperty / CSSOM assignment — never setAttribute('style', ...).
   ============================================================ */
...
/* ---------- globe palette — System Mug: ELECTRIC dots on VOID ----------
   [r,g,b] in 0..1. glow == VOID (#080C10) so the halo is killed. */
function palette(){
  return {
    base:[0.28,0.36,0.68],    // dim electric land dots (sphere body)
    marker:[0.353,0.471,1.0], // --accent ELECTRIC #5A78FF markers
    glow:[0.031,0.047,0.063],  // --ink VOID #080C10 -> KILLS the halo
...
/* ============================================================
   ...
   Colour by verdict tier, sized by severity. Projected DOM overlay.
   ============================================================ */
...
/* ============================================================
   GEO / verdict mock — drives the fly-to + escalation card.
   ============================================================ */
const GEO = {
  '185.220.101.34': {
...
/* ---------- projection: model-space r -> screen ---------- */
...
function projectPins(){
  ...
  if (!projW) return;
  const dim = (flying || cardShown) ? 0.10 : 1;
  for (const p of PINS){
    projectOne(p.r, _o);
...
/* ---------- hover hit-test + glass verdict tooltip ---------- */
...
const SS   = Math.max(ZMAX, FLY_ZOOM);
...
/* ============================================================
   FLY-TO — hand-rolled critically-damped springs on cobe's loop.
   ============================================================ */
const OMEGA = 7.2;
...
function flyBack(){
  ...
  flyPhi.x = phi;   flyPhi.v = 0;   flyPhi.t = phi;
  ...
  globe = createGlobe(canvas, {
    ...
    onRender(state){
      ...
      if (flying){
        ...
        else if (activeId >= 0) ts = 0;
        else if (hovering) ts = 0.30;
...
/* ---------- drag to rotate; drag overrides / interrupts fly ---------- */
function onDown(e){
  if (rmq.matches) return;
  dragging = true; lastX = e.clientX; pressMoved = false;
  if (flying && !flyBackMode){ flying = false; }
...
if (input){
  input.addEventListener('input', () => {
    syncDetect();
    if (cardShown && input.value.trim() === '') flyBack();
  });
