...
const GEO = {
  ...
  // Kept in sync by hand with shared/card/geo.ts — this file does not import it.
...
function geoModel(sources) {
  ...
  let code = "", city = "", asn = "", org = "", lat = null, lon = null, source = "";
  ...
  if (!code) {   // fall back to a reputation source's country code (AbuseIPDB)
    for (const s of sources ?? []) {
      ...
      if (cc && /^[A-Za-z]{2}$/.test(String(cc))) { code = String(cc).toUpperCase(); source = s.name ?? ""; break; }
    }
  }
...
function paintGeo(ctx, m, T, draw, y, inner) {
  ...
  if (draw) {
    ...
    // Capped at one decimal (~11km): even a wire-supplied coordinate is only
    // ever city-level (IP geolocation is never street-address precise), so a
    // 2-decimal (~1km) reading implied accuracy the source never actually held.
    const coord = g.precise
      ? `${Math.abs(g.lat).toFixed(1)}°${g.lat >= 0 ? "N" : "S"} ${Math.abs(g.lon).toFixed(1)}°${g.lon >= 0 ? "E" : "W"}`
      : "country-level";
    const via = g.source ? `via ${g.source}` : "source unknown";
    const attribution = g.precise ? `city-level · ${via}` : via;
    ctx.fillText(coord, boxX + boxW - ipad, gy + 12);
    ctx.fillText(attribution, boxX + boxW - ipad, gy + 24);
    ctx.textAlign = "left";
  }
